...
router.post('/eventData', function(req, res){
  ...
  pg.connect(connectionString, function(err, client, done){
    if(err){
      ...
      var getValQuery = ("SELECT events.id, event_name, address_one, events_state_province, company, begin_date, end_date, notes_events, results_url, schedule_url, hotel_name, hotel_url, event_city, event_url FROM events JOIN hotels ON events.hotel_id = hotels.id WHERE events.id =" + req.body.eventId);
